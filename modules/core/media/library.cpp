...
void Library::setItemState(const QModelIndex & ind, int state) {
    bool override = state == IItem::flag_not_liked;
    if (state & IItem::flag_listened || override)
        state = 0;
    else if (state & IItem::flag_liked)
        state = 1;
    else return;
...
void Library::stateRestoring(const QModelIndex & ind, QFutureWatcher<void> * watcher) {
        ...
        if (state != -1) {
            if (state == 1) {
                if (!watcher || (watcher && watcher -> isCanceled()))
                    itm -> set(IItem::flag_liked);
                else
                    emitItemAttrChanging(ind, IItem::flag_liked);
    ...
    if (isListened) {
        if (canceled)
            itm -> set(IItem::flag_listened);
        else
            emitItemAttrChanging(ind, IItem::flag_listened);
    } else
        if (!canceled)
            emitItemAttrChanging(ind, IItem::flag_new_item);
