...
//                lQuery(baseUrlStr(QStringLiteral("/Genre/Page") % OFFSET_TEMPLATE), proc_genres1, DEFAULT_REQUESTS_LIMIT);
            ...
            QJsonValue searchProc(const SearchLimit & limits) { // this should be rewritten
                QUrl url = QUrl(baseUrlStr(qst_site, search_path_token));
                ...
                if (!limits.by_artists() && songs_table) {
                    ...
                    for(Html::Set::Iterator song = songs.begin(); song != songs.end(); song++) {
                        ...
                        track_obj.insert(tkn_grab_refresh, baseUrlStr(qst_site, track_tag -> link()));
                ...
                switch(arg -> post_proc) {
                    case proc_tracks1: {
                        ...
                        for(Html::Set::Iterator track = tracks.begin(); track != tracks.end(); track++) {
                            ...
                            track_obj.insert(tkn_grab_url, baseUrlStr(qst_site, tag -> value(data_url_token)));
                            ...
                            if (!set.isEmpty())
                                track_obj.insert(tkn_grab_refresh, baseUrlStr(qst_site, set.link()));
